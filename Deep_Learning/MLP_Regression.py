...
model = Sequential([
    Dense(num_neurons_in_hidden_layers, input_dim=num_lags, activation='relu'),
    Dense(num_neurons_in_hidden_layers, activation='relu'),
    Dense(1)
])
...
y_predicted_train = model.predict(x_train).reshape(-1, 1)
